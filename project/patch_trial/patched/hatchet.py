# patched/hatchet.py
# replaces /app/core/providers/orchestration/hatchet.py
import asyncio
import logging
...
class HatchetOrchestrationProvider(OrchestrationProvider):
    ...
    async def start_worker(self):
        if not self.worker:
            raise ValueError("Worker not initialized. Call get_worker() first.")
    ...
    async def run_workflow(
        ...
        **kwargs,
        ...
        task_id = self.orchestrator.admin.run_workflow(
            workflow_name, parameters, options=options, *args, **kwargs
        )
        return {
            "task_id": str(task_id),
            "message": self.messages.get(workflow_name, "Workflow queued successfully."),
        }
    ...
    def register_workflows(
        ...
        logger.info(f"Registering workflows for {workflow} with messages {messages}.")

        if workflow == Workflow.INGESTION:
            from core.main.orchestration.hatchet.ingestion_workflow import hatchet_ingestion_factory
            workflows = hatchet_ingestion_factory(self, service)
        elif workflow == Workflow.GRAPH:
            from core.main.orchestration.hatchet.graph_workflow import hatchet_graph_search_results_factory
